...
(async()=>{
 ...
 await context.route('**/auth/v1/**',async route=>{
  ...
  await context.route('https://t1.daumcdn.net/**',route=>route.fulfill({contentType:'application/javascript',body:"window.daum={Postcode:function(options){this.open=function(config){window.__postcodeQuery=config&&config.q;options.oncomplete({zonecode:'06236',roadAddress:'서울특별시 강남구 테헤란로 123',roadAddressEnglish:'123, Teheran-ro, Gangnam-gu, Seoul'});};}};"}));
 ...
 await btn('학생 관리').click();await btn('학생 추가').click();await btn('학생 등록',dialog()).click();await dialog().getByText(/필수 항목을 입력해 주세요/).waitFor();assert(await dialog().locator('.is-required-empty').count()>=5);await dialog().getByLabel('성 *').fill('검증');await dialog().getByLabel('이름 *').fill('학생');await dialog().getByLabel('생년월일 *').fill('2012-03-04');await dialog().getByLabel('등록 프로그램 *').selectOption('시니어보딩');await dialog().getByLabel(/김 컨설턴트/).check();assert(await dialog().getByLabel(/검증 관리자/).isChecked());await page.screenshot({path:out+'/student-registration-required.png'});await dialog().getByRole('button',{name:'닫기'}).click();
 await page.getByLabel('업무 검증 학생 선택').first().check();await btn('학생 삭제').click();await dialog().getByRole('heading',{name:'학생 삭제'}).waitFor();assert(await btn('학생 삭제',dialog()).isDisabled());await dialog().getByLabel('확인 문구').fill('삭제');assert(!(await btn('학생 삭제',dialog()).isDisabled()));await btn('취소',dialog()).click();await page.getByLabel('업무 검증 학생 선택').first().uncheck();
 await btn('열기').click();await btn('미팅 기록').click();await btn('미팅 추가').click();
 assert.match(await dialog().getByLabel('미팅 제목 *').inputValue(),/^\d{2}\/\d{2}\/\d{4} 미팅$/);await dialog().getByLabel('미팅 제목 *').fill('학기 목표 점검');await dialog().getByLabel('오늘 목표').fill('수학 발표와 목표');await dialog().getByLabel('지난 미팅 이후 1').fill('지난 과제를 완료했습니다.');
 const mathActivity=dialog().locator('article').filter({hasText:'Math Club'});await mathActivity.getByLabel('진행 내용').fill('수학 활동 자료를 검토했습니다.');await dialog().getByLabel('내부 전용 메모').fill('내부 비공개 코칭 기록');
 await dialog().getByLabel('학부모 전달 요약').fill('수학 발표 준비가 순조롭게 진행되고 있습니다.');await dialog().getByLabel('학부모 공개 진행상황에 전달 요약 포함').check();await page.screenshot({path:out+'/meeting-record-structured.png',fullPage:true});
 await btn('할 일 추가',dialog()).click();await dialog().getByLabel('할 일 / 숙제').fill('9/24까지 발표 슬라이드 완성');assert.equal(await dialog().getByLabel('마감일').inputValue(),'2026-09-24');
 await btn('검토 완료 · 기록과 변경사항 반영',dialog()).click();await saved();await page.waitForTimeout(150);
 assert.equal(fixture.students[0].ecs.length,1);assert.equal(fixture.students[0].ecs[0].meetingProgressNote,'수학 활동 자료를 검토했습니다.');assert(fixture.students[0].tasks.some(t=>t.title==='9/24까지 발표 슬라이드 완성'&&t.deadline==='2026-09-24'));assert.equal(fixture.students[0].operations.meetings[0].todayGoal,'수학 발표와 목표');
 await btn('기록 보기').click();await btn('이 기록을 불러와 새 미팅 작성',dialog()).click();assert.equal(await dialog().getByLabel('오늘 목표').inputValue(),'수학 발표와 목표');await btn('임시저장 / 미팅 예약',dialog()).click();await saved();assert.equal(fixture.students[0].operations.meetings.length,2);assert(fixture.students[0].operations.meetings.some(m=>m.appliedAt));
 ...
 await btn('캘린더').click();await page.getByLabel('캘린더 월').fill('2026-09');await page.getByRole('button',{name:/9\/24까지 발표 슬라이드 완성/}).waitFor();
 ...
 await btn('Stage 1–5').click();await page.getByRole('button',{name:/Stage 5: 입학/}).click();await page.getByRole('heading',{name:'인터뷰 준비'}).waitFor();await page.getByText('Phillips Exeter Academy',{exact:true}).last().waitFor();await btn('에세이').click();await page.getByRole('heading',{name:'학교별 에세이'}).waitFor();await page.getByText('공식 포털에서 확인한 질문 테스트',{exact:true}).waitFor();
 ...
 await btn('Stage 1–5').click();await page.getByRole('button',{name:/Stage 1: 학생 분석/}).click();await page.getByLabel('주소 검색어').first().fill('테헤란로 123');await page.getByRole('button',{name:'도로명·건물명·지번 검색',exact:true}).first().click();await page.getByLabel('우편번호').first().waitFor();assert.equal(await page.getByLabel('우편번호').first().inputValue(),'06236');assert.equal(await page.getByLabel('한국어 주소').first().inputValue(),'서울특별시 강남구 테헤란로 123');assert.equal(await page.getByLabel('영문 주소').first().inputValue(),'123, Teheran-ro, Gangnam-gu, Seoul');assert.equal(await page.evaluate(()=>window.__postcodeQuery),'테헤란로 123');await saved();
