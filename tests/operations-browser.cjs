...
(async()=>{
 ...
 await dialog().getByLabel('미팅 제목 *').fill('학기 목표 점검');await dialog().getByLabel('오늘 목표').fill('수학 발표와 목표');await dialog().getByLabel('지난 미팅 이후').fill('지난 과제를 완료했습니다.');
 await dialog().getByLabel('Academics / 학업').fill('수학 발표의 근거 자료를 검토했습니다.');await dialog().getByLabel('Internal only — do not share / 내부 전용 메모').fill('내부 비공개 코칭 기록');
 await dialog().getByLabel('Summary to send parents / 학부모 전달 요약').fill('수학 발표 준비가 순조롭게 진행되고 있습니다.');await dialog().getByLabel('학부모 공개 진행상황에 전달 요약 포함').check();
 await dialog().getByRole('button',{name:'On track',exact:true}).first().click();await page.screenshot({path:out+'/meeting-record-structured.png',fullPage:true});
 await btn('할 일 추가',dialog()).click();await dialog().getByLabel('Task / Homework').fill('발표 슬라이드 완성');await dialog().getByLabel('Due date').fill('2026-09-24');
 await btn('검토 완료 · 기록과 변경사항 반영',dialog()).click();await saved();await page.waitForTimeout(150);
 assert.equal(fixture.students[0].ecs.length,1);assert(fixture.students[0].tasks.some(t=>t.title==='발표 슬라이드 완성'));assert.equal(fixture.students[0].operations.meetings[0].todayGoal,'수학 발표와 목표');
