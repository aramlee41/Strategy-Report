...
(async()=>{
 ...
 try{
  ...
  const data=await page.evaluate(()=>{
   ...
   return {students:[make('senior','시니어 학생','시니어보딩',['s','t'],[{id:'s-task',title:'김 담당 전용 업무',ownerId:'s',deadline:past},{id:'t-task',title:'이 담당 전용 업무',ownerId:'t',deadline:past},{id:'joint',title:'공동 관리 업무',deadline:day}]),make('junior','주니어 학생','주니어보딩',['t'],[{id:'j-task',title:'주니어 원서 확인',ownerId:'t',deadline:past}]),make('platinum','업무 검증 학생','플래티넘',['s'],Array.from({length:15},(_,i)=>({id:'many-'+i,title:'검증 업무 '+(i+1),ownerId:'s',deadline:past})))],schools:PREP_SCHOOLS,day};
  });
  data.students[0].operations.updates=Array.from({length:7},(_,i)=>({id:'notice-'+i,at:new Date(Date.now()-i*60000).toISOString(),title:'미팅 기록 반영 '+(i+1),detail:'다음 상담을 위한 자료가 준비되었습니다.'}));
  let staffTeams={empty:['대학원']},version=1,workspaceVersion=0,workspace={events:[],tasks:[{id:'personal',title:'관리자 개인 업무',date:data.day}],subscriptions:{team:true}};const versions={senior:1,junior:1,platinum:1,__settings:1};
  ...
  await ctx.route('**/functions/v1/prep-portal',async route=>{
   ...
   if(b.action==='load')response={...data,user:admin,staffAccounts:[admin,s,t,empty],staffTeams,versions,workspaceVersion,workspace,teamEvents:[],sharedCalendar:[]};
   else if(b.action==='revisions')response={versions,workspaceVersion};
   else if(b.action==='saveWorkspace'){assert.equal(b.version,workspaceVersion);workspace=b.payload;response={payload:workspace,version:++workspaceVersion};}
  ...
  await page.getByLabel('팀',{exact:true}).selectOption('플래티넘');assert.equal(await page.locator('.ops-task-table tbody tr').count(),12);await page.getByRole('button',{name:'업무 더 보기 (3)',exact:true}).click();assert.equal(await page.locator('.ops-task-table tbody tr').count(),15);
  await page.getByLabel('팀',{exact:true}).selectOption('시니어보딩');assert.equal(await page.locator('.update-row').count(),5);await button('업데이트 더 보기 (2)').click();assert.equal(await page.locator('.update-row').count(),7);await button('모두 확인').click();await page.getByText('공용 저장 완료',{exact:true}).waitFor();assert(workspace.readAt);assert.equal(await page.locator('.update-new').count(),0);
  ...
  const colors=await page.evaluate(()=>{const heading=document.querySelector('.ops-band h2'),table=document.querySelector('.ops-table thead th');return {bg:getComputedStyle(document.body).backgroundColor,sidebar:getComputedStyle(document.querySelector('.side')).backgroundColor,nav:getComputedStyle(document.querySelector('.navbtn.active')).backgroundColor,logo:document.querySelector('.brand-logo').naturalWidth,headingBg:getComputedStyle(heading.parentElement.matches('.ops-toolbar')?heading.parentElement:heading).backgroundColor,headingText:getComputedStyle(heading).color,tableBg:getComputedStyle(table).backgroundColor,tableText:getComputedStyle(table).color,tableBorder:getComputedStyle(table.closest('table')).borderLeftWidth,rowBg:getComputedStyle(document.querySelector('.ops-table tbody tr:nth-child(2)')).backgroundColor};});
  assert.equal(colors.bg,'rgb(255, 255, 255)');assert.equal(colors.sidebar,'rgb(36, 50, 82)');assert.equal(colors.nav,'rgb(243, 231, 201)');assert(colors.logo>0);assert.equal(colors.headingBg,'rgb(255, 255, 255)');assert.equal(colors.headingText,'rgb(36, 50, 82)');assert.equal(colors.tableBg,'rgb(52, 68, 97)');assert.equal(colors.tableText,'rgb(255, 255, 255)');assert.equal(colors.tableBorder,'1px');assert.equal(colors.rowBg,'rgb(247, 248, 250)');
  const textStyle=await page.locator('.ops-workspace .table tbody td:nth-child(2)').first().evaluate(el=>{const s=getComputedStyle(el);return {color:s.color,weight:s.fontWeight,size:s.fontSize};});assert.deepEqual(textStyle,{color:'rgb(32, 36, 43)',weight:'600',size:'16px'});
  await require('./workspace-design-check.cjs')(page,out);
