...
(async()=>{
 ...
 try{
  ...
  const colors=await page.evaluate(()=>({bg:getComputedStyle(document.body).backgroundColor,nav:getComputedStyle(document.querySelector('.navbtn.active')).backgroundColor,logo:document.querySelector('.brand-logo').naturalWidth}));assert.equal(colors.bg,'rgb(246, 246, 247)');assert.equal(colors.nav,'rgb(243, 231, 201)');assert(colors.logo>0);
