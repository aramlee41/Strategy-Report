...
(async()=>{
 ...
 try{
  ...
  const colors=await page.evaluate(()=>{const heading=document.querySelector('.ops-band h2'),table=document.querySelector('.ops-table thead th');return {bg:getComputedStyle(document.body).backgroundColor,nav:getComputedStyle(document.querySelector('.navbtn.active')).backgroundColor,logo:document.querySelector('.brand-logo').naturalWidth,headingBg:getComputedStyle(heading.parentElement.matches('.ops-toolbar')?heading.parentElement:heading).backgroundColor,headingText:getComputedStyle(heading).color,tableBg:getComputedStyle(table).backgroundColor,tableText:getComputedStyle(table).color};});assert.equal(colors.bg,'rgb(255, 255, 255)');assert.equal(colors.nav,'rgb(243, 231, 201)');assert(colors.logo>0);assert.equal(colors.headingBg,'rgb(36, 50, 82)');assert.equal(colors.headingText,'rgb(255, 255, 255)');assert.equal(colors.tableBg,'rgb(36, 50, 82)');assert.equal(colors.tableText,'rgb(255, 255, 255)');
