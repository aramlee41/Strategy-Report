(function(root) {
  ...
  function changesBetween(before, after) {
    ...
    for(const s of after.students||[]) if(JSON.stringify(previous.get(s.id))!==JSON.stringify(s)) changes.push({id:s.id,kind:'student',payload:s});
    ...
    if(JSON.stringify(before.teamEvents||[])!==JSON.stringify(after.teamEvents||[])) changes.push({id:'__settings',kind:'config',payload:{teamEvents:after.teamEvents||[]}});
    return changes;
  }
