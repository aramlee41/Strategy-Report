...
function StageOne({ st, update, schools, staff }) {
  ...
  const saveCustom = x => {
    if (!x.name) return;
    if (modal === "current") update({ school: x.name, currentSchoolInfo: { ...currentSchoolInfo, ...x } });
    if (modal?.startsWith("previous")) setPrev(Number(modal.replace("previous", "")), x);
    update({ customSchools: [...(st.customSchools || []), x] });
  };
  const tabs = [
    ["basic", "기본 정보"],
    ["program", "프로그램/목표"],
  ...
  return <div className="grid">
    <CustomSchoolModal open={!!modal} onClose={() => setModal(null)} onSave={saveCustom} />
    <div className="tabs">{tabs.map(t => <button key={t[0]} className={"tab " + (sub === t[0] ? "active" : "")} onClick={() => setSub(t[0])}>{t[1]}</button>)}</div>
    {sub === "basic" && <div className="grid">
      <div className="card"><h3>기본 정보</h3><div className="grid g4">
        <Input label="성" val={basic.lastNameKo} set={v => setBasic("lastNameKo", v)} />
        <Input label="이름" val={basic.firstNameKo} set={v => setBasic("firstNameKo", v)} />
        <Input label="영문 성" val={basic.lastNameEn} set={v => setBasic("lastNameEn", v)} />
        <Input label="영문 이름" val={basic.firstNameEn} set={v => setBasic("firstNameEn", v)} />
        <Input label="학생명" val={st.name} set={v => update({ name: v })} />
        <Input label="English Name" val={st.en} set={v => update({ en: v })} />
        <Input label="Preferred Name" val={basic.preferredName} set={v => setBasic("preferredName", v)} />
        <DateField label="생년월일" val={basic.dob} set={v => setBasic("dob", v)} />
        <HSelect label="성별" val={basic.gender} set={v => setBasic("gender", v)} options={["남자", "여자", "미입력"]} />
        <Input label="출생 도시" val={basic.birthCity} set={v => setBasic("birthCity", v)} />
        <Input label="출생 국가" val={basic.birthCountry} set={v => setBasic("birthCountry", v)} />
        <Input label="국적" val={basic.nationality} set={v => setBasic("nationality", v)} />
        <HSelect label="미국 영주권/시민권" val={basic.usStatus} set={v => setBasic("usStatus", v)} options={["없음", "영주권", "시민권", "기타"]} />
        <Input label="학생 이메일" val={basic.email} set={v => setBasic("email", v)} />
        <Input label="학생 휴대폰" val={basic.phone} set={v => setBasic("phone", v)} />
        <Input label="부모 연락처" val={basic.parentPhone} set={v => setBasic("parentPhone", v)} />
        <Input label="우편번호" val={basic.zip} set={v => setBasic("zip", v)} />
        <Input label="모국어" val={basic.firstLanguage} set={v => setBasic("firstLanguage", v)} />
        <Input label="가정 사용 언어" val={basic.homeLanguage} set={v => setBasic("homeLanguage", v)} />
        <Input label="소통 가능 언어" val={basic.languages} set={v => setBasic("languages", v)} />
        <HSelect label="Financial Aid" val={basic.financialAid} set={v => setBasic("financialAid", v)} options={["Yes", "No", "미정"]} />
        <HSelect label="Boarding/Day 지원" val={basic.boardingDay} set={v => setBasic("boardingDay", v)} options={["Boarding", "Day", "Both"]} />
      </div><Text label="현재 거주 주소" val={basic.address} set={v => setBasic("address", v)} /><Text label="추가 필수 정보/특이사항" val={basic.requiredNotes} set={v => setBasic("requiredNotes", v)} /></div>
      <div className="card"><h3>가족 정보</h3><div className="grid g3">
        <Input label="아버지 성명" val={family.fatherName} set={v => setFamily("fatherName", v)} />
        <Input label="아버지 직업/회사" val={family.fatherWork} set={v => setFamily("fatherWork", v)} />
        <Input label="아버지 이메일" val={family.fatherEmail} set={v => setFamily("fatherEmail", v)} />
        <Input label="어머니 성명" val={family.motherName} set={v => setFamily("motherName", v)} />
        <Input label="어머니 직업/회사" val={family.motherWork} set={v => setFamily("motherWork", v)} />
        <Input label="어머니 이메일" val={family.motherEmail} set={v => setFamily("motherEmail", v)} />
      </div><Text label="형제/자매" val={family.siblings} set={v => setFamily("siblings", v)} /><Text label="보딩스쿨 관련 가족/지인" val={family.boardingConnections} set={v => setFamily("boardingConnections", v)} /><Text label="가족/양육권/종교/기타 메모" val={family.notes} set={v => setFamily("notes", v)} /></div>
    </div>}
    {sub === "program" && <div className="grid">
      <div className="card"><h3>프로그램 / 목표 / 담당자</h3><div className="grid g3">
        <HSelect label="소속 프로그램" val={st.program} set={v => update({ program: v })} options={PROGRAM_OPTIONS} />
        <HSelect label="현재 학년" val={st.currentGrade} set={v => update({ currentGrade: v, grade: v })} options={GRADE_OPTIONS} />
        <HSelect label="지원 연도" val={st.targetYear} set={v => update({ targetYear: v })} options={YEAR_OPTIONS} />
        <HSelect label="지원 학년" val={st.targetGrade} set={v => update({ targetGrade: v })} options={TARGET_GRADE_OPTIONS} />
        <DateField label="프로그램 종료일" val={st.programEndDate} set={v => update({ programEndDate: v, deadline: v })} />
        <HSelect label="Stage" val={st.stage} set={v => update({ stage: v })} options={STAGE_KEYS.map(x => x[0])} />
      </div><MultiStaff staff={staff} selected={st.owners || []} set={owners => update({ owners, owner: owners[0] })} /></div>
      <InterestSchools st={st} update={update} schools={schools} />
    </div>}
    {sub === "schools" && <SchoolBlocks st={st} update={update} schools={schools} currentSchoolInfo={currentSchoolInfo} setCurrentSchoolInfo={setCurrentSchoolInfo} previousSchools={previousSchools} setPrev={setPrev} setModal={setModal} />}
    {sub === "grades" && <Academics st={st} update={update} />}
    {sub === "tests" && <FinalTests st={st} update={update} />}
    {sub === "ecs" && <FinalEcs st={st} update={update} />}
    {sub === "report" && <BasicReport st={st} schools={schools} />}
  </div>;
}
