...
function BasicReport({ st, schools }) {
  ...
  return <div className="card"><h3>1차 기초 보고서</h3><p className="small">현재까지 입력한 정보 기준의 내부 전략 진단입니다. 합격 보장이 아니라, 입력 근거와 불확실성을 함께 보는 초기 보고서입니다.</p><div className="rubrics">{Object.entries(r).slice(0, 4).map(([k, v]) => <Rub key={k} title={k} val={v.score} max={v.max} />)}</div><table className="table"><tbody><tr><th>장점</th><td>{Object.entries(r).filter(([k, v]) => v.score / v.max >= .7).map(([k]) => k).join(", ") || "추가 입력 필요"}</td></tr><tr><th>보완점</th><td>{Object.entries(r).filter(([k, v]) => v.score / v.max < .6).map(([k, v]) => `${k}: ${v.gap}`).join(" / ") || "큰 약점 없음"}</td></tr><tr><th>관심 학교 가능성</th><td>{interestRecs.map(x => `${x.name}: ${x.tier} (${x.fit}/100)`).join(" / ") || "관심 학교를 입력하면 표시됩니다."}</td></tr></tbody></table><EnhancedReport st={st} schools={interestRecs.length ? interestRecs : schools} /></div>;
}
