...
function CRMSettings({st,update,close}){
  ...
  return <PPDialog title="학생 관리 설정" close={close}><div className="ops"><div className="ops-grid"><OpsField label="CRM 관리 상태" value={d.status} options={CRM_STATUS} onChange={v=>set('status',v)}/><OpsField label="관리 우선순위" value={d.priority} options={[["standard","일반"],["high","우선 관리"]]} onChange={v=>set('priority',v)}/><OpsField label="정기 연락 주기" value={String(d.cadence)} options={[7,14,30,60].map(n=>[String(n),n+'일'])} onChange={v=>set('cadence',Number(v))}/><OpsField label="선호 연락 수단" value={d.preferredChannel} options={CRM.channels} onChange={v=>set('preferredChannel',v)}/></div>
    ...
    <OpsField label="담당자 인수인계 메모 (내부)" type="textarea" value={d.handoff} onChange={v=>set('handoff',v)}/><OpsSave run={()=>update(CRM.settings(st,{status:d.status,priority:d.priority,cadence:d.cadence,preferredChannel:d.preferredChannel,tags:d.tags,handoff:d.handoff||''}))} onDone={close}/>
  </div></PPDialog>;
}
