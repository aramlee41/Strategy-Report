...
function PPManager({ data, persist, user }) {
  ...
  const saveParent = (id, p, action) => {
    if (data.cloudConnected) throw new Error("미리보기에서는 저장하지 않습니다. 실제 학부모 계정으로 로그인해 입력해 주세요.");
    const original = students.find(s => s.id === id);
    if (!original || !PP.normalize(original).enabled) throw new Error("이 학생의 포털이 활성화되지 않았습니다.");
    return persist({ ...data, students: data.students.map(s => s.id === id ? { ...s, parentPortal: p } : s) });
  };
  ...
  const finishReview = async accepted => {
    ...
    try { await persist({ ...data, students: data.students.map(s => s.id === st.id ? { ...s, ...profile, parentPortal: p } : s) }); setReview(null); setMessage(accepted ? "기존 Stage 1 학생 정보에 반영했습니다." : "보완 요청을 등록했습니다."); } catch(e) { setMessage(e.message); }
  };
  const release = async () => {
    ...
    try { await update({ ...portal, publications: [p, ...portal.publications], updates: [{ id: p.id, at: now, title: "새 기초 보고서가 공개되었습니다", detail: p.title }, ...portal.updates] }); setPublish(false); setMessage("현재 보고서를 학부모에게 배포했습니다."); } catch(e) { setMessage(e.message); }
  };
  return <div className="portal-manager"><h2>학부모 포털 관리</h2><p className="portal-muted">학생 자료 제출을 검토하고, 학부모에게 공개할 보고서와 입금 안내를 관리합니다.</p>{data.cloudConnected ? <div className="portal-notice">공용 저장소에 연결되었습니다. 초대한 학부모에게 연결된 자녀 자료와 승인된 보고서만 공개됩니다.</div> : <div className="portal-notice warn">현재는 같은 브라우저에서 확인하는 미리보기입니다. 실제 계정으로 로그인하면 공용 저장소를 이용할 수 있습니다.</div>}<label className="field"><span className="label">학생 선택</span><select className="select" value={st.id} onChange={e => { setSelected(e.target.value); setMessage(""); }}>{students.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select></label>
    ...
    {data.cloudConnected && <PPCloudAccounts key={st.id + "invite"} fixedStudent={st} students={[st]} admin={user.role === "admin"} />}
...
function PPEntry() {
  if (window.PREP_CLOUD_CONFIG && new URLSearchParams(location.search).get("mode") !== "local") return <PPCloudApp />;
...
// portal-cloud.jsx mounts after both the local and cloud views are defined.
