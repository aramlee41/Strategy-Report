...
function MeetingAcademicRows({rows,setRows,onAction}){
  const change=(index,patch)=>setRows(rows.map((row,i)=>i===index?{...row,...patch}:row));
  return <div className="meeting-linked-block"><h4>학업 · 수강 과목 진행상황</h4>{rows.map((row,index)=><MeetingLinkedRow key={row.id} title={row.subject} meta={`${row.school||'학교 미입력'} · ${row.termLabel||'학기 미입력'}`} row={row} change={patch=>change(index,patch)} onAction={onAction}/>)}{!rows.length&&<OpsEmpty>학교/학업에서 과목을 등록하면 이곳에 자동으로 표시됩니다.</OpsEmpty>}</div>;
}
function MeetingTestRows({rows,setRows,onAction}){
  const change=(index,patch)=>setRows(rows.map((row,i)=>i===index?{...row,...patch}:row));
  const add=()=>setRows([...rows,{id:`new-test-${O.id()}`,newGoal:true,type:'',target:'',nextDate:'',progressStatus:'',progressNote:'',issue:''}]);
  return <div className="meeting-linked-block"><div className="ops-toolbar"><h4>시험 / 준비 목표</h4><OpsButton icon="Plus" onClick={add}>시험 목표 추가</OpsButton></div>{rows.map((row,index)=><MeetingLinkedRow key={row.id} title={row.type||'새 시험 목표'} meta={row.target!==''?`목표 ${row.target}`:'목표 점수 미설정'} row={row} change={patch=>change(index,patch)} onAction={onAction}><OpsField label="시험" value={row.type||''} options={MEETING_TEST_TYPES} onChange={value=>change(index,{type:value})}/><OpsField label="목표 점수" value={row.target??''} onChange={value=>change(index,{target:value})}/><OpsField label="준비 상태" value={row.progressStatus||''} options={MEETING_PROGRESS_OPTIONS.slice(1)} onChange={value=>change(index,{progressStatus:value})}/><OpsField label="준비 진행상황" value={row.progressNote||''} onChange={value=>change(index,{progressNote:value})}/><OpsField label="문제 상황" value={row.issue||''} onChange={value=>change(index,{issue:value})}/><OpsField label="다음 시험일" type="date" value={row.nextDate||''} onChange={value=>change(index,{nextDate:value})}/></MeetingLinkedRow>)}{!rows.length&&<OpsEmpty>시험 목표 또는 시험 기록을 추가해 주세요.</OpsEmpty>}</div>;
}
function MeetingActivityRows({rows,setRows,onAction}){
  const change=(id,patch)=>setRows(rows.map(row=>row.id===id?{...row,...patch}:row));
  return <div className="meeting-linked-block"><h4>활동 진행상황</h4>{MEETING_ACTIVITY_GROUPS.map(([group,label])=><section className="meeting-activity-group" key={group}><h5>{label}</h5>{rows.filter(row=>row.group===group).map(row=><MeetingLinkedRow key={row.id} title={row.name} meta={row.category} row={row} change={patch=>change(row.id,patch)} onAction={onAction}/>)}{!rows.some(row=>row.group===group)&&<p className="ops-muted">{label}에 등록된 진행 중 활동이 없습니다.</p>}</section>)}</div>;
}
function MeetingInterviewRows({rows,setRows,onAction}){
  const change=(index,patch)=>setRows(rows.map((row,i)=>i===index?{...row,...patch}:row));
  return <div className="meeting-linked-block"><h4>인터뷰</h4>{rows.map((row,index)=><MeetingLinkedRow key={row.id} title={row.school||'지원 학교'} meta={row.date||'예약일 미정'} row={row} change={patch=>change(index,patch)} onAction={onAction}><OpsField label="인터뷰 일정" type="date" value={row.date||''} onChange={value=>change(index,{date:value})}/><OpsField label="예약 상태" value={row.status||''} options={['미예약','예약 완료','준비 중','완료']} onChange={value=>change(index,{status:value})}/><OpsField label="준비 상태" value={row.prepStatus||''} options={MEETING_PROGRESS_OPTIONS.slice(1)} onChange={value=>change(index,{prepStatus:value})}/><OpsField label="진행 / 준비 기록" value={row.notes||''} onChange={value=>change(index,{notes:value})}/><OpsField label="Google Doc / 자료 링크" type="url" value={row.docUrl||''} onChange={value=>change(index,{docUrl:value})}/>{opsUrl(row.docUrl)&&<OpsLink url={row.docUrl}>인터뷰 문서 열기</OpsLink>}</MeetingLinkedRow>)}{!rows.length&&<OpsEmpty>지원 학교를 등록하면 인터뷰 항목이 자동으로 표시됩니다.</OpsEmpty>}</div>;
}
function MeetingEssayRows({rows,setRows,onAction}){
  const change=(index,patch)=>setRows(rows.map((row,i)=>i===index?{...row,...patch}:row));
  return <div className="meeting-linked-block"><h4>에세이</h4>{rows.map((row,index)=><MeetingLinkedRow key={row.id} title={`${row.school} · ${row.title}`} meta={row.prompt} row={row} change={patch=>change(index,patch)} onAction={onAction}><OpsField label="마감일" type="date" value={row.deadline||''} onChange={value=>change(index,{deadline:value})}/><OpsField label="우선순위" value={row.priority||''} options={['높음','보통','낮음']} onChange={value=>change(index,{priority:value})}/><OpsField label="진행 상태" value={row.status||''} options={OPS_STATUS} onChange={value=>change(index,{status:value})}/><OpsField label="진행 기록" value={row.progressNote||''} onChange={value=>change(index,{progressNote:value})}/><OpsField label="Google Doc / 초안 링크" type="url" value={row.docUrl||''} onChange={value=>change(index,{docUrl:value})}/>{opsUrl(row.docUrl)&&<OpsLink url={row.docUrl}>에세이 문서 열기</OpsLink>}</MeetingLinkedRow>)}{!rows.length&&<OpsEmpty>원서의 에세이 질문을 등록하면 이곳에 자동으로 표시됩니다.</OpsEmpty>}</div>;
}
function MeetingActionItems({items,setItems,staff,studentName,meetingDate}){
  const add=()=>setItems([...items,{id:O.id(),title:'',responsibleId:'student',responsibleName:studentName||'학생',dueDate:'',dueDateAuto:true,done:false,url:'',createTask:true}]);
  const change=(index,patch)=>setItems(items.map((item,i)=>i===index?{...item,...patch}:item));
  const titleChange=(index,value)=>{const current=items[index],parsed=window.PrepMeetingRecords.parseDueDate(value,meetingDate);change(index,{title:value,...((!current.dueDate||current.dueDateAuto)&&parsed?{dueDate:parsed,dueDateAuto:true}:{})});};
  const people=[['student',`${studentName||'학생'} (학생)`],...staff.map(person=>[person.id,person.name])];
  return <div className="meeting-actions"><div className="ops-toolbar"><h4>다음 미팅 전 할 일</h4><OpsButton icon="Plus" onClick={add}>할 일 추가</OpsButton></div>{items.map((item,index)=><article className="meeting-action-row" key={item.id||index}><OpsField label="할 일 / 숙제" placeholder="예: 9/21까지 초안 작성하고 제출하기" value={item.title} onChange={value=>titleChange(index,value)}/><OpsField label="담당" value={item.responsibleId||'student'} options={people} onChange={value=>change(index,{responsibleId:value,responsibleName:value==='student'?(studentName||'학생'):(staff.find(person=>person.id===value)?.name||'')})}/><OpsField label="마감일" type="date" value={item.dueDate} onChange={value=>change(index,{dueDate:value,dueDateAuto:false})}/><div className="meeting-action-options"><OpsCheck label="완료" value={item.done} onChange={value=>change(index,{done:value})}/><OpsCheck label="할 일·캘린더에 반영" value={item.createTask!==false} onChange={value=>change(index,{createTask:value})}/><OpsButton icon="Trash2" aria-label="할 일 삭제" onClick={()=>setItems(items.filter((_,i)=>i!==index))}/></div><div className="meeting-action-link"><OpsField label="자료 링크 (선택)" type="url" placeholder="https://" value={item.url} onChange={value=>change(index,{url:value})}/>{opsUrl(item.url)&&<OpsLink url={item.url}>자료 열기</OpsLink>}</div></article>)}{!items.length&&<OpsEmpty>할 일 추가를 눌러 한 줄씩 입력해 주세요. 날짜 표현을 찾으면 마감일을 자동으로 채웁니다.</OpsEmpty>}</div>;
}
