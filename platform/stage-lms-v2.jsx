...
function v2ReadableValue(value, fallback = "입력 전") {
  return value || value === 0 ? value : fallback;
}
function v2ReadableFitLabel(fit) {
  const map = {
    Excellent: "매우 우수",
    Strong: "우수",
    Competitive: "경쟁 가능",
    "Borderline Competitive": "보완 필요",
    "Below Target": "목표 대비 부족",
    "Significantly Below Target": "큰 폭의 보완 필요",
    "Insufficient Data": "판단 보류"
  };
  return map[fit] || fit || "판단 보류";
}
function v2ReadableSchoolTier(school = {}) {
  const tier = v2SchoolTestTier(school);
  if (tier === "top") return "최상위권";
  if (tier === "selective") return "상위권";
  return "중상위권";
}
function v2ClientRubrics(st) {
  const legacy = v2LegacyStage1Score(st);
  const academicSummary = v2AcademicSummary(st);
  const comments = academicSummary.flatMap(a => String(a.comment || "").split("/").map(x => x.trim()).filter(Boolean));
  const ecNames = (st.ecs || []).map(e => e.name).filter(Boolean).slice(0, 3).join(", ");
  return [
    {
      key: "academics",
      title: "학업 준비도",
      score: legacy.academicsA,
      evidence: `성적표, SSAT, 영어 시험 데이터를 함께 반영했을 때 학업 준비도는 ${legacy.academicsA}점입니다. SSAT는 ${v2ReadableValue(legacy.ssatRaw)}, 영어 시험은 ${legacy.toefl.label}, 성적 평가는 ${legacy.gpa.label} 기준으로 보았습니다.`,
      gap: legacy.academicsA >= 85 ? "현재 학업 지표는 지원서에서 강점으로 사용할 수 있습니다. 다만 상위권 보딩스쿨에서는 과목별 성적 흐름과 Teacher's Comment가 함께 설득력을 만들어 주므로, 최근 2~3년의 핵심 과목 변화와 교사의 구체 코멘트를 계속 정리해 두는 것이 좋습니다." : "학업 준비도는 아직 보완 여지가 있습니다. 특히 핵심 과목의 성적 변화, SSAT와 영어 시험의 세부 영역, 교사 코멘트를 함께 정리해야 학교가 학생의 학업 잠재력을 더 명확하게 이해할 수 있습니다."
    },
    {
      key: "english",
      title: "영어 및 커뮤니케이션",
      score: legacy.toefl.eval ?? 0,
      evidence: `영어 지표는 ${legacy.toefl.label} 기준으로 보았습니다. 보딩스쿨은 시험 점수뿐 아니라 인터뷰에서 자신의 관심사, 학교 적합성, 성장 경험을 자연스럽게 설명하는 힘을 함께 평가합니다.`,
      gap: (legacy.toefl.eval ?? 0) >= 90 ? "영어 점수는 기본 경쟁력을 갖춘 편입니다. 이제는 인터뷰 답변의 구조, 구체적인 사례, Why School 표현을 다듬어 점수가 실제 소통력으로 이어진다는 인상을 주는 것이 중요합니다." : "영어 영역은 점수와 말하기 증빙을 함께 보완해야 합니다. 시험 점수 향상과 동시에 인터뷰에서 사용할 핵심 경험 5~6개를 영어로 정리해 두면 지원서 전체의 설득력이 좋아집니다."
    },
    {
      key: "boarding",
      title: "보딩 적합도",
      score: legacy.boardingB,
      evidence: `보딩 적합도는 영어 소통력, 인터뷰에서 보일 성숙도, 기숙사 생활에 필요한 자기관리 근거를 함께 반영했습니다. 현재 입력된 일정, 활동, 학생 스토리를 기준으로 ${legacy.boardingB}점입니다.`,
      gap: legacy.boardingB >= 80 ? "기숙사 생활 적응력은 긍정적으로 설명할 수 있습니다. 지원서에서는 책임감, 공동체 기여, 시간관리 사례를 구체적으로 보여 주면 좋습니다." : "보딩 적합도는 아직 서류에서 충분히 보이지 않을 수 있습니다. 주중 루틴, 방학 계획, 팀 활동에서의 책임감, 선생님이나 코치의 관찰 코멘트를 보강해 학생이 독립적으로 생활할 준비가 되어 있음을 보여 주어야 합니다."
    },
    {
      key: "ec",
      title: "EC 깊이와 차별성",
      score: legacy.cocurricularC,
      evidence: `EC는 활동 개수만 보지 않고 기간, 역할, 성과, 학생만의 스토리로 이어지는지를 보았습니다. 현재 대표 활동은 ${ecNames || "아직 충분히 정리되지 않았습니다"}이며, 이를 기준으로 ${legacy.cocurricularC}점입니다.`,
      gap: legacy.cocurricularC >= 80 ? "EC는 지원서의 핵심 차별점으로 사용할 수 있습니다. 이제 활동별 성과를 숫자, 수상, 포트폴리오, 추천서 문장으로 증빙하는 작업이 필요합니다." : "EC는 단순 참여보다 깊이와 결과물이 더 필요합니다. 한두 개 활동을 집중적으로 발전시켜 팀 내 역할, 외부 성과, 지속 기간, 학생만의 관점을 분명히 만들어야 합니다."
    },
    {
      key: "recommendation",
      title: "추천서 및 근거 강도",
      score: Math.min(100, Math.max(35, legacy.boardingFit + comments.length * 4)),
      evidence: comments.length ? `성적표와 교사 코멘트에서 ${comments.slice(0, 2).join(" / ")} 등의 근거를 확인했습니다.` : "아직 추천서에 활용할 만한 Teacher's Comment나 교사 관찰 문장이 충분히 입력되지 않았습니다.",
      gap: comments.length >= 2 ? "추천서에 사용할 수 있는 근거가 어느 정도 확보되어 있습니다. 과목별로 학생의 태도, 성장, 기여가 반복적으로 드러나도록 문장 후보를 정리하면 설득력이 높아집니다." : "추천서는 합격 가능성을 끌어올리는 중요한 자료입니다. 과목 교사, 코치, 어드바이저가 학생을 어떻게 설명할 수 있는지 구체 문장과 사례를 미리 수집해야 합니다."
    }
  ];
}
function V2RadarChart({ st }) {
  ...
  return <div style={{ display: "grid", gridTemplateColumns: "minmax(430px, 500px) 1fr", gap: 18, alignItems: "center" }}>
    ...
    <div>
      <h3 style={{ marginTop: 0 }}>학생 역량 방사형 분석</h3>
      <p className="small" style={{ fontSize: 13, lineHeight: 1.8 }}>이 그래프는 학업, 영어, 보딩 적합도, EC 깊이, 리더십, Hook을 한눈에 비교하기 위한 자료입니다. 바깥쪽에 가까운 축은 현재 지원서에서 강점으로 사용할 수 있고, 안쪽에 머무는 축은 남은 기간 동안 보완 전략을 세워야 하는 영역입니다.</p>
      <table className="table"><tbody>{items.map(x => <tr key={x.label}><th>{x.label}</th><td><div className="progress"><div style={{ width: `${Math.max(0, Math.min(100, x.value))}%` }} /></div></td><td style={{ width: 50 }}>{v2Round(x.value, 1)}</td></tr>)}</tbody></table>
    </div>
  </div>;
}
function v2CustomerSchoolNote(prediction) {
  const c = prediction.category || "";
  if (/Safety|Likely/i.test(c)) return "현재 입력된 자료만 놓고 보면 비교적 현실적인 지원권에 들어옵니다. 다만 보딩스쿨 지원에서는 학교별 에세이, 인터뷰 완성도, 추천서의 구체성이 최종 결과를 크게 좌우하므로 안정권으로만 보지 말고 Why School을 분명히 준비해야 합니다.";
  if (/Competitive/i.test(c)) return "학생의 현재 준비도와 학교의 기대 수준이 비교적 맞닿아 있는 구간입니다. 합격 가능성을 높이려면 학생의 강점이 해당 학교가 원하는 학생상과 직접 연결되어야 하며, 추천서와 인터뷰에서도 같은 메시지가 반복되어야 합니다.";
  if (/Goal/i.test(c)) return "도전적인 목표 학교입니다. 현재 점수만으로는 보완해야 할 부분이 있으므로 시험 세부 영역, 리더십 성과, 인터뷰 스토리를 남은 기간 동안 우선적으로 끌어올리는 전략이 필요합니다.";
  return "상위 도전 학교입니다. 지원 자체는 가능하지만 현재 자료만으로는 합격 설득력이 충분하지 않을 수 있습니다. 강력한 Hook, 뚜렷한 성과, 추천서, 학교별 에세이 완성도가 반드시 필요합니다.";
}
function v2GeneralTestAnalysis(st) {
  const tests = st.tests || [];
  if (!tests.length) return { summary: "아직 입력된 시험 점수가 없어 시험 기반 분석은 보류합니다.", strengths: [], weaknesses: ["SSAT 또는 영어 공인시험 점수를 입력하면 목표 학교별로 더 구체적인 해석이 가능합니다."], next: ["가장 최근 공식 리포트의 총점, 세부 점수, 응시일을 먼저 입력해 주세요."] };
  const ssat = v2LatestTest(st, /SSAT/i);
  const english = v2LatestTest(st, /TOEFL|IELTS|DET/i);
  const sat = v2LatestTest(st, /^(SAT|ACT|PSAT)$/i);
  const strengths = [];
  ...
  if (ssat) {
    const d = ssat.details || {};
    const p = v2PrimaryTestScore(ssat);
    if (p) strengths.push(`SSAT는 Overall Percentile ${p}를 기준으로 해석했습니다. 보딩 지원에서는 원점수보다 percentile이 학생의 상대적 위치를 더 잘 보여 주므로, 보고서에서도 percentile을 우선 반영합니다.`);
    if (v2Num(d["Quantitative Percentile"]) >= 95) strengths.push("Quantitative 영역이 강하면 Math/STEM 계열 수업 적응력과 학업 잠재력을 설명하는 좋은 근거가 됩니다.");
    if (v2Num(d["Verbal Percentile"]) < 88 || v2Num(d["Reading Percentile"]) < 88) weaknesses.push("Verbal 또는 Reading percentile이 낮다면 영어 독해량이 많은 수업, 토론형 수업, humanities 계열 과목에서의 적응력을 별도 자료로 보완하는 것이 좋습니다.");
    next.push("다음 시험에서는 Verbal과 Reading의 안정성을 높이고, Quantitative 강점은 관심 학교의 STEM 프로그램과 연결해 지원 스토리로 정리해 주세요.");
  }
  if (english) {
    const d = english.details || {};
    const score = v2PrimaryTestScore(english);
    strengths.push(`${english.type} ${score}점은 영어로 수업을 따라갈 수 있는 기본 준비도를 보여주는 자료입니다. 다만 학교별 기대 수준이 다르므로 관심 학교의 경쟁도와 함께 해석해야 합니다.`);
    if (/TOEFL/i.test(english.type || "")) {
      if (v2Num(d.Speaking) < 24) weaknesses.push("Speaking 점수가 낮으면 인터뷰와 토론 참여에서 보완 설명이 필요할 수 있습니다.");
      if (v2Num(d.Writing) < 25) weaknesses.push("Writing 점수가 낮은 경우 English 또는 History처럼 글쓰기 비중이 큰 과목에서의 준비도를 교사 코멘트, writing sample, 최근 성적 흐름으로 함께 보여 주는 것이 좋습니다.");
      next.push("영어 시험은 총점뿐 아니라 Speaking/Writing처럼 학교생활 적응과 직접 연결되는 영역을 함께 관리해 주세요.");
    }
  }
  if (sat) {
    const score = v2PrimaryTestScore(sat);
    strengths.push(`${sat.type} ${score}점은 고학년 지원자에게 학업 역량을 보조적으로 설명할 수 있는 자료입니다.`);
  }
  return { summary: "시험 점수는 단순히 높고 낮음으로만 판단하기보다, 세부 영역의 균형과 목표 학교의 경쟁 수준을 함께 보아야 합니다.", strengths, weaknesses, next };
}
function v2SchoolSpecificTestAnalyses(st, schools = []) {
  const interestNames = (st.interests || []).map(x => x.school).filter(Boolean);
  const tests = (st.tests || []).filter(t => v2PrimaryTestScore(t));
  if (!interestNames.length || !tests.length) return [];
  return interestNames.map(name => {
    const school = v2FindSchool(schools, name) || { name };
    const comments = tests.map(test => {
      const type = String(test.type || "").toUpperCase();
      ...
      const benchmark = v2TestingBenchmark(school, type);
      const fit = v2TestFit(score, benchmark);
      const tierText = v2ReadableSchoolTier(school);
      const unit = type === "SSAT" ? " percentile" : "점";
      let extra = "";
      const d = test.details || {};
      if (type === "TOEFL" && v2Num(d.Speaking) && v2Num(d.Speaking) < 24) extra += " 특히 Speaking은 인터뷰와 토론식 수업 적응력을 보여 주는 영역이므로 보완 관리가 필요합니다.";
      if (type === "SSAT" && benchmark.recommended && (v2Num(d["Verbal Percentile"]) < benchmark.recommended || v2Num(d["Reading Percentile"]) < benchmark.recommended)) extra += " Verbal/Reading은 학교 수준 대비 보완 여지가 있으므로 reading-heavy 수업에 적응할 수 있다는 근거를 함께 준비해 주세요.";
      if (!benchmark.recommended) return `${test.type} ${score}${unit}은 참고 자료로 확인했습니다. 이 학교의 시험 기대 수준을 더 정확히 보려면 학교별 최신 요구사항 확인이 필요합니다.`;
      return `${school.name}은 ${tierText} 경쟁군에 속하므로 ${test.type} ${score}${unit}은 현재 ${v2ReadableFitLabel(fit)} 수준으로 볼 수 있습니다. 1차 목표는 ${benchmark.recommended}${unit}, 더 안정적인 목표는 ${benchmark.competitive}${unit} 이상으로 잡는 것이 좋습니다.${extra}`;
    });
    const firstFit = tests[0] ? v2TestFit(v2PrimaryTestScore(tests[0]), v2TestingBenchmark(school, tests[0].type)) : "Insufficient Data";
    return { school, fit: firstFit, comments };
  });
}
function V2TestAnalysisReport({ st }) {
  const general = v2GeneralTestAnalysis(st);
  return <div>
    <div className="section-title"><span>01-2</span>시험 점수 해석</div>
    <div className="card" style={{ background: "#f8fbfe" }}>
      <h3>시험 점수 종합 해석</h3>
      <p style={{ lineHeight: 1.8 }}>{general.summary}</p>
      <div className="grid g3">
        <div><b>강점</b>{(general.strengths.length ? general.strengths : ["아직 뚜렷한 시험 강점이 입력되지 않았습니다."]).map((x, i) => <p className="small" key={i}>{x}</p>)}</div>
        <div><b>보완점</b>{(general.weaknesses.length ? general.weaknesses : ["현재 입력값만으로는 뚜렷한 보완점이 보이지 않습니다. 관심 학교별 기준 확인이 필요합니다."]).map((x, i) => <p className="small" key={i}>{x}</p>)}</div>
        <div><b>다음 목표</b>{(general.next.length ? general.next : ["관심 학교의 경쟁 수준을 기준으로 다음 시험 목표를 설정해 주세요."]).map((x, i) => <p className="small" key={i}>{x}</p>)}</div>
      </div>
    </div>
  </div>;
}
function V2ClientStrategyReport({ st, schools }) {
  ...
  const interestNames = (st.interests || []).map(x => x.school).filter(Boolean);
  const interestPredictions = interestNames.length ? v2LegacyPredictions(st, reportSchools) : [];
  const schoolTestRows = v2SchoolSpecificTestAnalyses(st, reportSchools);
  const schoolTestMap = Object.fromEntries(schoolTestRows.map(row => [row.school.name, row]));
  const appliedLegacy = interestPredictions.filter(p => p.legacyImpact?.applied && p.legacyContribution > 0);
  const recommended = v2RecommendedSchools(st, window.PREP_SCHOOLS || reportSchools);
  const lowest = [...rubrics].sort((a, b) => a.score - b.score).slice(0, 2);
  ...
  return <div className="report">
    <div className="report-cover">
      <div className="brand">YES Boarding Prep</div>
      <h2>{studentName} 학생 종합 전략 리포트</h2>
      <p>{st.school || "현재 학교 미입력"} · {st.targetYear || "지원연도 미정"} {st.targetGrade || ""} · 입력 자료와 기존 합격 분석 모델 기반</p>
    </div>
    <div className="report-body">
      <div className="section-title"><span>01</span>학생 기본 진단</div>
      <div className="grid g4">
      ...
      <p style={{ lineHeight: 1.8 }}>현재까지 입력된 자료를 기준으로 보면 {studentName} 학생의 종합 준비도는 <b>{legacy.weighted}점</b>입니다. 이 점수는 합격을 보장하는 숫자가 아니라, 학업·시험·보딩 적합도·EC를 나누어 보고 남은 기간의 우선순위를 정하기 위한 내부 분석 지표입니다.</p>
      <V2RadarChart st={st} />
      ...
      <div className="grid g2">
        ...
        <div className="card" style={{ background: "#ffffff" }}>
          <h3>시험 점수 그래프</h3>
          <p className="small muted">입력된 SSAT, TOEFL, IELTS, DET 등 주요 시험 점수를 시험별 기준에 맞춰 시각화합니다.</p>
          <V2TestScoreChart st={st} />
        </div>
      </div>
      ...
      <div className="section-title"><span>02</span>Rubric 점수표와 근거</div>
      <div className="rubrics">{rubrics.map(r => <Rub key={r.key} title={r.title} val={v2Round(r.score, 1)} max={100} />)}</div>
      <table className="table" style={{ marginTop: 14 }}><tbody>{rubrics.map(r => <tr key={r.key}><th style={{ width: 160 }}>{r.title}</th><td><p style={{ margin: 0, lineHeight: 1.75 }}>{r.evidence}</p><p style={{ margin: "8px 0 0", lineHeight: 1.75 }}><b>보완 방향:</b> {r.gap}</p></td></tr>)}</tbody></table>

      <div className="section-title"><span>03</span>관심/희망학교 판정</div>
      {interestPredictions.length ? <table className="table"><thead><tr><th>학교</th><th>현재 판정</th><th>시험 적합도</th><th>종합 해석 및 보완 방향</th></tr></thead><tbody>{interestPredictions.map(p => {
        const testRow = schoolTestMap[p.school.name];
        return <tr key={p.school.name}>
          <td><b>{p.school.name}</b><br /><span className="small muted">Yes Rank {p.school.yesRank || "-"} · 합격률 {p.school.accept || "-"}%</span></td>
          <td><V2ClientCategoryPill category={p.category} /></td>
          <td>{testRow ? <><V2ClientCategoryPill category={testRow.fit} /><p className="small muted" style={{ marginTop: 6 }}>{v2ReadableFitLabel(testRow.fit)}</p></> : <span className="small muted">시험 데이터 추가 필요</span>}</td>
          <td style={{ lineHeight: 1.75 }}><p style={{ margin: 0 }}>{v2CustomerSchoolNote(p)}</p>{testRow?.comments?.length ? <div style={{ marginTop: 8 }}><b>시험 해석:</b>{testRow.comments.map((c, i) => <p key={i} className="small" style={{ lineHeight: 1.75, margin: "6px 0 0" }}>{c}</p>)}</div> : <p className="small muted" style={{ margin: "8px 0 0" }}>SSAT 또는 영어 시험 세부 점수를 입력하면 이 학교 기준의 시험 적합도까지 함께 표시됩니다.</p>}</td>
        </tr>;
      })}</tbody></table> : <p className="small">관심/희망학교가 아직 지정되지 않았습니다. 학생 상세에서 관심학교를 입력하면 해당 학교만 대상으로 판정과 시험 적합도가 표시됩니다.</p>}

      {appliedLegacy.length > 0 && <><div className="section-title"><span>03-1</span>가족/졸업생 관계 영향</div>{appliedLegacy.map(p => <p key={p.school.name} style={{ lineHeight: 1.8 }}><b>{p.school.name}</b><br />입력된 가족 또는 졸업생 관계는 보조적인 긍정 요소로 반영했습니다. 다만 이 요소는 합격을 보장하지 않으며, 학업 준비도, 학생의 고유한 강점, 인터뷰, 추천서, 학교별 에세이 완성도를 대신할 수는 없습니다.</p>)}</>}

      <div className="section-title"><span>04</span>현재 점수 기준 추천 학교 4개</div>
      <table className="table"><thead><tr><th>추천 학교</th><th>추천 구간</th><th>추천 이유</th><th>다음 액션</th></tr></thead><tbody>{recommended.map(p => <tr key={p.school.name}><td><b>{p.school.name}</b><br /><span className="small muted">{p.school.state || ""} · SSAT 기준 {p.school.ssat || "-"} · 합격률 {p.school.accept || "-"}%</span></td><td><V2ClientCategoryPill category={p.category} /></td><td style={{ lineHeight: 1.7 }}>현재 입력된 준비도에서 현실적으로 검토할 수 있는 학교입니다. {p.school.fit || "학업 적합도, 보딩 준비도, EC 깊이를 함께 검토해야 합니다."}</td><td style={{ lineHeight: 1.7 }}>{p.school.name}의 프로그램 중 학생의 Hook과 연결되는 지점을 2개 이상 찾고, 인터뷰와 에세이에서 사용할 구체 사례를 준비해 주세요.</td></tr>)}</tbody></table>

      <div className="section-title"><span>05</span>우선 보완 전략</div>
      {lowest.map((r, i) => <p key={r.key} style={{ lineHeight: 1.8 }}><b>{i + 1}. {r.title}</b><br />{r.gap}</p>)}
      <p style={{ lineHeight: 1.8 }}>남은 기간에는 모든 항목을 동시에 넓게 보완하기보다, 점수가 낮은 축을 중심으로 증빙 가능한 결과물을 만드는 것이 중요합니다. 특히 학교별 에세이와 인터뷰에서는 활동을 단순히 나열하기보다, 학생이 어떤 환경에서 성장했고 어떤 방식으로 학교 공동체에 기여할 수 있는지를 한 문장으로 정리해야 합니다.</p>
    </div>
  </div>;
}
