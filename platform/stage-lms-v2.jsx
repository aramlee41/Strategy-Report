...
function V2StageOne({ st, update, schools, staff }) {
  ...
  return <div><V2SubTabs tabs={tabs} active={sub} set={setSub} />{sub === "identity" && <V2Identity basic={basic} setBasic={setBasic} />}{sub === "program" && <V2Program st={st} update={update} staff={staff} />}{sub === "schools" && <V2SchoolInfo st={st} update={update} schools={schools} />}{sub === "grades" && <V2Transcript st={st} update={update} />}{sub === "tests" && <V2Tests st={st} update={update} />}{sub === "ecs" && <V2Ecs st={st} update={update} />}{sub === "report" && <V2BasicReport st={st} schools={schools} />}</div>;
}
function V2Identity({ basic, setBasic }) {
  return <div className="grid"><V2Section title="이름 / 생년월일"><div className="grid g4"><V2Field label="성" val={basic.lastNameKo} set={v => setBasic("lastNameKo", v)} /><V2Field label="이름" val={basic.firstNameKo} set={v => setBasic("firstNameKo", v)} /><V2Field label="영문 이름" val={basic.firstNameEn} set={v => setBasic("firstNameEn", v)} /><V2Field label="영문 성" val={basic.lastNameEn} set={v => setBasic("lastNameEn", v)} /><V2Field label="Preferred Name" val={basic.preferredName} set={v => setBasic("preferredName", v)} /><V2Field label="생년월일" type="date" val={basic.dob} set={v => setBasic("dob", v)} /><V2Select label="성별" val={basic.gender} set={v => setBasic("gender", v)} options={["남자", "여자", "미입력"]} /></div></V2Section><V2Section title="출생 / 국적"><div className="grid g3"><V2Field label="출생 도시" val={basic.birthCity} set={v => setBasic("birthCity", v)} /><V2Select label="출생 국가" val={basic.birthCountry} set={v => setBasic("birthCountry", v)} options={V2_COUNTRIES} />{basic.birthCountry === "기타" && <V2Field label="출생 국가 직접 입력" val={basic.birthCountryOther} set={v => setBasic("birthCountryOther", v)} />}</div><V2Multi label="국적" values={basic.nationalities || []} set={v => setBasic("nationalities", v)} options={V2_NATIONALITIES} otherValue={basic.nationalityOther} setOther={v => setBasic("nationalityOther", v)} /><V2Select label="미국 영주권/시민권" val={basic.usStatus} set={v => setBasic("usStatus", v)} options={["없음", "영주권", "시민권", "기타"]} /></V2Section><V2Section title="연락처 / 언어"><div className="grid g3"><V2Field label="학생 이메일" val={basic.email} set={v => setBasic("email", v)} /><V2Field label="학생 휴대폰" val={basic.phone} set={v => setBasic("phone", v)} /><V2Field label="부모 연락처" val={basic.parentPhone} set={v => setBasic("parentPhone", v)} /><V2Field label="우편번호" val={basic.zip} set={v => setBasic("zip", v)} /></div><V2Multi label="모국어" values={basic.firstLanguages || []} set={v => setBasic("firstLanguages", v)} options={V2_LANGUAGES} otherValue={basic.firstLanguageOther} setOther={v => setBasic("firstLanguageOther", v)} /><V2Multi label="가정 사용 언어" values={basic.homeLanguages || []} set={v => setBasic("homeLanguages", v)} options={V2_LANGUAGES} otherValue={basic.homeLanguageOther} setOther={v => setBasic("homeLanguageOther", v)} /><V2Multi label="소통 가능 언어" values={basic.communicationLanguages || []} set={v => setBasic("communicationLanguages", v)} options={V2_LANGUAGES} otherValue={basic.communicationLanguageOther} setOther={v => setBasic("communicationLanguageOther", v)} /></V2Section><V2Section title="지원 조건"><div className="grid g3"><V2Select label="Financial Aid" val={basic.financialAid} set={v => setBasic("financialAid", v)} options={["Yes", "No", "미정"]} /><V2Select label="Boarding/Day 지원" val={basic.boardingDay} set={v => setBasic("boardingDay", v)} options={["Boarding", "Day", "Both"]} /></div><V2Text label="현재 거주 주소" val={basic.address} set={v => setBasic("address", v)} /><V2Text label="추가 필수 정보/특이사항" val={basic.requiredNotes} set={v => setBasic("requiredNotes", v)} /></V2Section></div>;
}
function V2Program({ st, update, staff }) {
  const toggleOwner = id => {
    const owners = st.owners || [];
    update({ owners: owners.includes(id) ? owners.filter(x => x !== id) : [...owners, id], owner: owners[0] || id });
  };
  return <div className="grid g2"><V2Section title="프로그램 / 지원 목표"><div className="grid g3"><V2Select label="소속 프로그램" val={st.program} set={v => update({ program: v })} options={V2_PROGRAM_OPTIONS} /><V2Select label="현재 학년" val={st.currentGrade} set={v => update({ currentGrade: v, grade: v })} options={V2_GRADE_OPTIONS} /><V2Select label="지원 연도" val={st.targetYear} set={v => update({ targetYear: v })} options={V2_YEAR_OPTIONS} /><V2Select label="지원 학년" val={st.targetGrade} set={v => update({ targetGrade: v })} options={V2_TARGET_GRADE_OPTIONS} /><V2Field label="프로그램 종료일" type="date" val={st.programEndDate} set={v => update({ programEndDate: v, deadline: v })} /><V2Select label="Stage" val={st.stage} set={v => update({ stage: v })} options={V2_STAGE_KEYS.map(x => x[0])} /></div></V2Section><V2Section title="담당자 복수 지정">{staff.map(a => <label key={a.id} className="small" style={{ display: "inline-flex", gap: 6, marginRight: 14, marginBottom: 10 }}><input type="checkbox" checked={(st.owners || []).includes(a.id)} onChange={() => toggleOwner(a.id)} />{a.name}</label>)}</V2Section></div>;
}
