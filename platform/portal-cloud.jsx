...
function PPCloudAccounts({ students = [], fixedStudent, admin = false }) {
  ...
  return <section className="portal-band"><h2>{fixedStudent?"학부모 계정 초대":"계정 / 초대 관리"}</h2><div className="grid g3"><V2Field label="이름" val={name} set={setName}/><V2Field label="이메일" type="email" val={email} set={setEmail}/>{admin&&!fixedStudent&&<label className="field"><span className="label">권한</span><select className="select" value={role} onChange={e=>setRole(e.target.value)}><option value="parent">학부모</option><option value="staff">컨설턴트</option><option value="admin">관리자</option></select></label>}</div>{role==="parent"&&!fixedStudent&&<fieldset><legend>연결할 자녀</legend>{students.map(s=><label key={s.id} style={{display:"block",padding:8}}><input type="checkbox" checked={selected.includes(s.id)} onChange={e=>setSelected(e.target.checked?[...selected,s.id]:selected.filter(x=>x!==s.id))}/> {s.name}</label>)}</fieldset>}<button className="btn primary" disabled={busy} onClick={invite}>{busy?"생성 중…":"초대 링크 만들기"}</button><p role="status">{message}</p>{link&&<div className="portal-notice"><label className="label">일회용 초대 링크</label><input className="input" readOnly value={link} onFocus={e=>e.target.select()}/><button className="btn ghost" onClick={async()=>{try{await navigator.clipboard.writeText(link);setMessage("초대 링크를 복사했습니다.");}catch(e){setMessage("초대 링크 입력창의 주소를 선택해 복사해 주세요.");}}}>링크 복사</button></div>}
    <h3>초대 현황</h3>{list.invitations.map(i=><div className="portal-row" key={i.id}><div><b>{i.name}</b><p>{i.email} · {i.role==="parent"?"학부모":i.role==="admin"?"관리자":"컨설턴트"} · {{pending:"초대 대기",reserved:"연결 중",accepted:"연결 완료",revoked:"취소"}[i.status]}</p></div>{["pending","reserved"].includes(i.status)&&<button className="btn ghost" onClick={async()=>{try{await PC.call({action:"revokeInvitation",id:i.id});await refresh();setLink("");}catch(e){setMessage(e.message);}}}>초대 취소</button>}</div>)}
    {admin&&<><h3>등록 계정</h3>{list.members.map(m=><div className="portal-row" key={m.user_id}><div><b>{m.name}</b><p>{m.email} · {m.role} · {m.active?"사용 중":"중지"}</p></div><button className="btn ghost" onClick={async()=>{if(!window.confirm(m.active?"이 계정의 LMS 접근을 중지할까요?":"이 계정의 접근을 다시 허용할까요?"))return;try{await PC.call({action:"disableAccount",id:m.user_id,disabled:m.active});await refresh();}catch(e){setMessage(e.message);}}}>{m.active?"이용 중지":"다시 허용"}</button></div>)}</>}
  </section>;
}
function PPCloudApp() {
  ...
  }catch(e){setError(e.message);}finally{setReady(true);setBusy(false);}};
  React.useEffect(()=>{
  ...
  if(!loaded)return <><PPCloudLogin inviteToken={inviteToken} onReady={()=>{setInviteToken(null);refresh();}}/>{error&&<div className="portal-notice warn" role="alert">{error}<button className="btn ghost" onClick={refresh}>다시 연결</button></div>}</>;
  ...
  return <><div className="cloud-status"><span role="status">{status}</span><div className="portal-actions">{loaded.user.role==="admin"&&<button className="btn ghost" disabled={busy||!!pending.current} onClick={importLocal}>기존 브라우저 자료 가져오기</button>}<button className="btn ghost" disabled={busy||!!pending.current} onClick={()=>{if(!blocked.current||window.confirm("저장하지 못한 변경을 닫고 공용 자료를 다시 불러올까요?"))refresh();}}>공용 자료 새로 불러오기</button></div></div>{error&&<div className="portal-notice warn" role="alert">{error}</div>}<V2App key={epoch} cloud={{data:loaded,user:loaded.user,save,logout}}/></>;
}
